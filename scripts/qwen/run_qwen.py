...
Adapted from scripts/run.py for the TrustNLP camera-ready cross-model
generalization experiment. Differences from the original Gemma script:
  - Loads Qwen/Qwen3.5-9B (HF token not required).
  - Layer path is model.model.layers (not model.model.language_model.layers).
  - SAE is loaded via sae_lens, exactly as in the original Gemma script,
    only the release name and sae_id format differ:
        release  = "qwen-scope-3.5-9b-base-w64k-l100"
        sae_id   = f"layer{N}"   (Gemma used "layer_{N}_width_{W}_l0_{L0}")
  - Iterates over ALL datasets in a single run so the model is loaded once.

PRE-REQUISITE: warm the Hugging Face cache once on a node with internet
access (e.g. the NCHC login node) before submitting the compute job:
    huggingface-cli download Qwen/SAE-Res-Qwen3.5-9B-Base-W64K-L0_100
or simply run a one-line python:
    python -c "from sae_lens import SAE; \\
        SAE.from_pretrained(release='qwen-scope-3.5-9b-base-w64k-l100', \\
                            sae_id='layer0')"
sae_lens then resolves all subsequent layers from ~/.cache/huggingface offline.
"""
